...
def benchmark_detritus(step_count: int = 1_000, population_size: int = 64) -> BenchmarkResult:
    """Measure plant-enabled population steps with detritus recycling enabled."""
    if isinstance(step_count, bool) or step_count <= 0:
        raise ValueError("step_count must be a positive integer")
    if isinstance(population_size, bool) or population_size <= 0:
        raise ValueError("population_size must be a positive integer")
    ...
    world = PopulationWorld(
        ...
        detritus_config=DetritusConfig(
            enabled=True,
            initial_detritus=1_000.0,
            maximum_decay_rate=1.0,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--mode",
        choices=("noop", "sandbox", "population", "plants", "detritus"),
        default="sandbox",
    )
